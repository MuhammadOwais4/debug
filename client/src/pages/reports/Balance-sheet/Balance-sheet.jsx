...
function BalanceSheet() {
  ...
  const loadVendors = async () => {
    try {
      ...
      const response = await fetch(`https://everyday-medline-somerset-timber.trycloudflare.com/api/chart-of-accounts/liabilities`);
  ...
  const loadClosingStock = async () => {
    try {
      ...
      const response = await fetch(`https://everyday-medline-somerset-timber.trycloudflare.com/api/products/get-stock`);
  ...
  const loadNetProfit = async () => {
    try {
      ...
      const response = await fetch(
        `https://everyday-medline-somerset-timber.trycloudflare.com/api/profit-loss?fromDate=${startDate}&toDate=${endDate}`
      );
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
      const data = await response.json();
      if (!data.success) {
  ...
  const loadTaxAccountBalances = async () => {
    ...
    for (const tax of TAX_CODES) {
      try {
        ...
        const response = await fetch(
          `https://everyday-medline-somerset-timber.trycloudflare.com/api/ledgers/account-ledger?${params}`
        );
        const data = await response.json();
        let balance = 0;
  ...
  const loadBalanceSheetData = async () => {
    try {
      ...
      const accountsResponse = await fetch(`https://everyday-medline-somerset-timber.trycloudflare.com/api/ledgers/accounts`);
